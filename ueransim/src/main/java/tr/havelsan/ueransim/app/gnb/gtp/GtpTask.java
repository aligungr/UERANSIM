...
public class GtpTask extends NtsTask {
    ...
    protected void main() {
        ...
        try {
            ctx.socket = new DatagramSocket(ctx.gnbCtx.config.gtpPort, InetAddress.getByName(ctx.gnbCtx.config.gtpIp));

        } catch (Exception e) {
            Log.error(Tag.CONN, "Failed to bind UDP/GTP socket %s:%s (%s)", ctx.gnbCtx.config.gtpIp, ctx.gnbCtx.config.gtpPort, e.toString());
            return;
        }
