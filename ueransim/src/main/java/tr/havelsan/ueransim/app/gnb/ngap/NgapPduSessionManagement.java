...
public class NgapPduSessionManagement {
    ...
    public static void receiveResourceSetupRequest(GnbSimContext ctx, NGAP_PDUSessionResourceSetupRequest message) {
        ...
        if (failed == 0) Log.success(Tag.RES, "PDU Session Establishment is successful");
        else if (succeeded == 0) Log.error(Tag.RES, "PDU Session Establishment is failed");
        else Log.info(Tag.RES, "PDU Session Establishment is partially successful.");
