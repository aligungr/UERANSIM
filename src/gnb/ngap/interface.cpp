...
void NgapTask::sendNgSetupRequest(int amfId)
{
    ...
    if (CellAccessTypeToRatInformation(m_base->config->cellAccessType, ratInformation))
    {
        ...
        // RAT-Information is defined CRITICALITY reject in SupportedTAItem-ExtIEs
        // (3GPP TS 38.413); a receiver that cannot comprehend it must reject the
        // NG Setup rather than silently treat the cell as terrestrial.
        ext->criticality = ASN_NGAP_Criticality_reject;
        ...
        asn::SequenceAdd(*extContainer, ext);
    }
